...
namespace robot_planner
{
  ...
  public:
...

    {
      ...
      path_action_client_ = rclcpp_action::create_client<nav2_msgs::action::NavigateThroughPoses>(this, "navigate_through_poses");
    ...
    void timer_callback()
    {
      ...
      if (!this->path_action_client_->wait_for_action_server())
      {
        RCLCPP_INFO(this->get_logger(), "Reached inside the error section");
      ...
      for (long unsigned int i = 0; i < paths[0].size(); ++i)
      {
        geometry_msgs::msg::PoseStamped pose;
        pose.header.frame_id = "map";
        pose.pose.position.x = paths[0][i].x;
        pose.pose.position.y = paths[0][i].y;
        pose.pose.orientation.w = 1.0;
        goal_msg.poses.push_back(pose);
      }
      RCLCPP_INFO(this->get_logger(), "Sending path goal");

      auto path_goal_options = rclcpp_action::Client<nav2_msgs::action::NavigateThroughPoses>::SendGoalOptions();
      path_goal_options.goal_response_callback =
          std::bind(&RobotPlanner::path_goal_response_callback, this, std::placeholders::_1);
      path_goal_options.feedback_callback =
          std::bind(&RobotPlanner::path_feedback_callback, this, std::placeholders::_1, std::placeholders::_2);
      path_goal_options.result_callback =
          std::bind(&RobotPlanner::path_result_callback, this, std::placeholders::_1);
      RCLCPP_INFO(this->get_logger(), "Sending path for following");
  ...
  private:
    rclcpp_action::Client<nav2_msgs::action::NavigateThroughPoses>::SharedPtr path_action_client_;
    ...
    void path_result_callback(const GoalHandleNavigateThroughPoses::WrappedResult &result)
    {
      ...
      case rclcpp_action::ResultCode::CANCELED:
        RCLCPP_ERROR(this->get_logger(), "Path following canceled");
        return;
      default:
